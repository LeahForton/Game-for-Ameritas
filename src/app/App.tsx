...
interface PlayerRow {
  name: string; emoji: string; capital: number; rbc: number; status: RBCStatus;
}
...
// ─── Data: Strategies ─────────────────────────────────────────────────────────
const STRATEGIES = [
  { id:"conservative", emoji:"🛡", name:"Safe Choice",     sub:"Grow slowly, stay secure", capitalGain:"+$5", riskLevel:2, risk:"Low Risk",    recommended:false,
    grad:"linear-gradient(145deg,#052e16,#064e3b)", border:"#16a34a", tag:"#bbf7d0", tagText:"#14532d", col:"#4ade80" },
  { id:"balanced",     emoji:"⚖️", name:"Balanced Choice", sub:"Grow steadily, manage risk", capitalGain:"+$10", riskLevel:3, risk:"Medium Risk", recommended:true,
    grad:"linear-gradient(145deg,#0f172a,#1e3a5f)", border:"#3b82f6", tag:"#bfdbfe", tagText:"#1e3a8a", col:"#60a5fa" },
  { id:"aggressive",   emoji:"🚀", name:"Risky Choice",    sub:"Grow fastest, highest stakes", capitalGain:"+$20", riskLevel:5, risk:"High Risk",   recommended:false,
...
const getStrategyCapitalDelta = (id: string) =>
  id === "balanced" ? 10 : id === "aggressive" ? 20 : 5;
...
const LEADERBOARD: PlayerRow[][] = [
  [
    {name:"Pinnacle Life", emoji:"🦅", capital:109,rbc:83,status:"healthy"},
    {name:"Guardian Re",   emoji:"🐺", capital:105,rbc:81,status:"healthy"},
    {name:"Frontier Life", emoji:"🐉", capital:105,rbc:80,status:"healthy"},
    {name:"Apex Shield",   emoji:"🦁", capital:105,rbc:80,status:"healthy"},
    {name:"Summit Mutual", emoji:"🐻", capital:105,rbc:80,status:"watch"},
    {name:"Coastal Assure",emoji:"🌊", capital:105,rbc:80,status:"watch"},
  ],
  [
    {name:"Pinnacle Life", emoji:"🦅", capital:89, rbc:86,status:"watch"},
    {name:"Frontier Life", emoji:"🐉", capital:90, rbc:83,status:"healthy"},
    {name:"Guardian Re",   emoji:"🐺", capital:90, rbc:82,status:"healthy"},
    {name:"Apex Shield",   emoji:"🦁", capital:97, rbc:81,status:"healthy"},
    {name:"Coastal Assure",emoji:"🌊", capital:95, rbc:80,status:"healthy"},
    {name:"Summit Mutual", emoji:"🐻", capital:100,rbc:80,status:"healthy"},
  ],
  [
    {name:"Pinnacle Life", emoji:"🦅",capital:84, rbc:89,status:"watch"},
    {name:"Frontier Life", emoji:"🐉",capital:72, rbc:86,status:"intervention"},
    {name:"Apex Shield",   emoji:"🦁", capital:85, rbc:84,status:"watch"},
    {name:"Guardian Re",   emoji:"🐺", capital:95, rbc:83,status:"healthy"},
    {name:"Coastal Assure",emoji:"🌊", capital:100,rbc:81,status:"healthy"},
    {name:"Summit Mutual", emoji:"🐻", capital:110,rbc:80,status:"healthy"},
  ],
  [
    {name:"Summit Mutual", emoji:"🐻", capital:95, rbc:80, status:"healthy"},
    {name:"Coastal Assure",emoji:"🌊", capital:85, rbc:81, status:"healthy"},
    {name:"Apex Shield",   emoji:"🦁", capital:72, rbc:85, status:"watch"},
    {name:"Guardian Re",   emoji:"🐺", capital:76, rbc:84, status:"watch"},
    {name:"Pinnacle Life", emoji:"🦅",capital:69, rbc:92, status:"intervention"},
    {name:"Frontier Life", emoji:"🐉",capital:57, rbc:89, status:"intervention"},
  ],
  [
    {name:"Summit Mutual", emoji:"🐻", capital:97, rbc:92, status:"healthy"},
    {name:"Coastal Assure",emoji:"🌊", capital:87, rbc:93, status:"watch"},
    {name:"Apex Shield",   emoji:"🦁",capital:75, rbc:95, status:"watch"},
    {name:"Guardian Re",   emoji:"🐺",capital:75, rbc:96, status:"watch"},
    {name:"Pinnacle Life", emoji:"🦅",capital:55, rbc:100,status:"intervention"},
    {name:"Frontier Life", emoji:"🐉",capital:44, rbc:97, status:"intervention"},
  ],
];

const P_STATS = [
  {capital:100, rbc:80,rep:70},
  {capital:105, rbc:80,rep:75},
  {capital:97, rbc:81,rep:72},
  {capital:85, rbc:84,rep:68},
  {capital:72, rbc:85,rep:65},
  {capital:75,rbc:95,rep:72},
];
...
const PlayerDashboard = ({ step, companyName, yourMoney, players, playerId }: { step:DemoStep; companyName:string; yourMoney:number; players:LobbyPlayer[]; playerId:string|null }) => {
  if (!step.round) return null;
  const r = step.round;
  const after = ["rbc","orsa","leaderboard"].includes(step.id);
  const statsIdx = after ? r : r - 1;
  const s = { ...P_STATS[Math.min(statsIdx, 5)], capital: yourMoney };
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,zIndex:30,
      ...
      <div style={{display:"flex",gap:"12px",alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
        {[{icon:<MoneyIcon color="#60a5fa" />,lbl:"Money Left",val:s.capital}].map(it=>(
...
// ─── Screen: Round Intro ──────────────────────────────────────────────────────
const RoundIntroScreen = ({round,viewMode,onNext,yourMoney}:{round:number;viewMode:ViewMode;onNext:()=>void;yourMoney:number}) => (
...
// ─── Screen: RBC — Player ─────────────────────────────────────────────────────
const RBCPlayerScreen = ({round,yourMoney}:{round:number;yourMoney:number}) => {
  const s = { ...P_STATS[round], capital: yourMoney };
  const status = statusOf(s.capital, s.rbc);
...
const LeaderboardScreen = ({round,viewMode,companyName,players,playerId}:{round:number;viewMode:ViewMode;companyName:string;players:LobbyPlayer[];playerId:string|null}) => {
  ...
  const normalizedHostName = companyName.trim().toLowerCase();
  const rankedPlayers = viewMode === "host"
    ? players.filter((player) => {
        const normalizedPlayerName = player.name.trim().toLowerCase();
        return player.id !== playerId
          && player.id !== "host"
          && normalizedPlayerName !== "host"
          && (!normalizedHostName || normalizedPlayerName !== normalizedHostName);
      })
    : players;
...
const FinalScreen = ({viewMode,choices,players,yourMoney,playerId,companyName}:{viewMode:ViewMode;choices:(string|null)[];players:LobbyPlayer[];yourMoney:number;playerId:string|null;companyName:string}) => {
  const rbcReq = P_STATS[5]?.rbc ?? 80;
  // The room API stores the host alongside players, but older API responses
  // do not always expose its ID as `playerId`. Exclude it by both ID and the
  // host's entered name before calculating final ranks.
  const normalizedHostName = companyName.trim().toLowerCase();
  const leaderboardPlayers = viewMode === "host"
    ? players.filter((player) => {
        const normalizedPlayerName = player.name.trim().toLowerCase();
        return player.id !== playerId
          && player.id !== "host"
...
export default function App() {
  ...
  const handleHost = async (name: string) => {
    ...
    try {
      ...
      const returnedHostId = result?.playerId ?? result?.id ?? result?.hostId ?? null;
      setPlayerId(returnedHostId);
      // The host controls the room but is not a competitor. The polling request
      // may still return its backend record; leaderboard components filter it.
      setPlayers([]);
      setYourMoney(100);
  ...
  const handleStrategy = (id:string) => {
    ...
    console.log("Choice / Money Delta:", { choice:id, moneyDelta });
  ...
  const renderScreen = () => {
    switch (step.id) {
      ...
      case "round-intro":
        return <RoundIntroScreen round={r} viewMode={selectedViewMode} yourMoney={yourMoney} onNext={advance} />;
      case "decision":
      ...
      case "rbc":
        return selectedViewMode === "host" ? <RBCHostScreen round={r} /> : <RBCPlayerScreen round={r} yourMoney={yourMoney} />;
      case "orsa":
      ...
      case "final":
        return <FinalScreen viewMode={selectedViewMode} choices={choices} players={players} yourMoney={yourMoney} playerId={playerId} companyName={companyName} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", fontFamily: F, position: "relative", background: "#0d1117" }}>
      ...
      {showDash && <PlayerDashboard step={step} companyName={companyName} yourMoney={yourMoney} players={players} playerId={playerId} />}
